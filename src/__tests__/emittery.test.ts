...
test('on()', async t => {
  ...
  await emitter.emit('🦄');
  await emitter.emit(eventName as any);
...
test.serial('events()', async t => {
  ...
  t.deepEqual(await iterator.next(), {done: true} as any);
});
...
test('events() - return() without argument', async t => {
  const emitter = new Emittery();
  const iterator = emitter.events('🦄');
  t.deepEqual(await iterator.return?.(), {done: true} as any);
});
...
test('off() - eventName must be a string or a symbol', t => {
  ...
  t.throws(
    () => {
      emitter.off(42 as any, undefined as any);
    },
    {instanceOf: TypeError},
  );
});
...
test('once() - eventName must be a string or a symbol', async t => {
  ...
  await t.throwsAsync(emitter.once(42 as any), {instanceOf: TypeError});
});
...
test('emit() - eventName must be a string or a symbol', async t => {
  ...
  emitter.emit('string');
  emitter.emit(Symbol('symbol') as any);

  await t.throwsAsync(emitter.emit(42 as any), {instanceOf: TypeError});
});
...
test('emitSerial() - eventName must be a string or a symbol', async t => {
  ...
  emitter.emitSerial(Symbol('symbol') as any);

  await t.throwsAsync(emitter.emitSerial(42 as any), {instanceOf: TypeError});
});
...
test('onAny() - must have a listener', t => {
  ...
  t.throws(
    () => {
      emitter.onAny(undefined as any);
    },
    {instanceOf: TypeError},
  );
});
...
test('offAny() - no listener', t => {
  ...
  t.throws(
    () => {
      emitter.offAny(undefined as any);
    },
    {instanceOf: TypeError},
  );
});
...
test('clearListeners() - also clears iterators', async t => {
  ...
  t.deepEqual(await iterator.next(), {done: true} as any);
  t.deepEqual(await anyIterator.next(), {done: false, value: ['🦄', '💫']});
  t.deepEqual(await anyIterator.next(), {done: true} as any);
});
...
test('listenerCount() - eventName must be undefined if not a string nor a symbol', t => {
  ...
  t.throws(
    () => {
      emitter.listenerCount(42 as any);
    },
    {instanceOf: TypeError},
  );
});
...
test('bindMethods() - methodNames must be array of strings or undefined', t => {
  t.throws(() => {
    new Emittery().bindMethods({}, null as any);
  });
...
test('bindMethods() - target must be an object', t => {
  const emitter = new Emittery();
  t.throws(() => emitter.bindMethods('string' as any, []));
  t.throws(() => emitter.bindMethods(null as any, []));
  t.throws(() => emitter.bindMethods(undefined as any, []));
});
...
test('mixin()', t => {
  ...
  const instance = new TestClassWithMixin(symbol) as any;
...
test('mixin() - methodNames must be array of strings or undefined', t => {
  ...
  t.throws(() => Emittery.mixin('emitter', null as any)(TestClass));
  t.throws(() => Emittery.mixin('emitter', 'string' as any)(TestClass));
  t.throws(() => Emittery.mixin('emitter', {} as any)(TestClass));
  t.throws(() => Emittery.mixin('emitter', [null] as any)(TestClass));
  t.throws(() => Emittery.mixin('emitter', [1] as any)(TestClass));
  t.throws(() => Emittery.mixin('emitter', [{}] as any)(TestClass));
});
...
test('mixin() - target must be function', t => {
  t.throws(() => Emittery.mixin('emitter')('string' as any));
  t.throws(() => Emittery.mixin('emitter')(null as any));
  t.throws(() => Emittery.mixin('emitter')(undefined as any));
  t.throws(() => Emittery.mixin('emitter')({} as any));
});
